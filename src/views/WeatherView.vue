...
<template>
  <div class="row row-cols-1 row-cols-md-3 g-4">
    <div class="col" v-for="(i,index) in All_travel_list " :key="index">
      <div class="card h-100" style="background-color: rgb(90, 163, 139);">
        <img :src="i.img" class="card-img-top" alt="...">
        <div class="card-body" style="background-color: rgb(109, 183, 207);">
          <h5 class="card-title"><strong>{{i.name}}</strong></h5>
          <p class="card-text">CHECK WATHER</p>
        </div>
        <div class="card-footer">
          <RouterLink :to="`/WeatherCard/${index}`">
            <button class="btn btn-primary" type="submit" style="background-color: black;">CHECK</button>
          </RouterLink>
